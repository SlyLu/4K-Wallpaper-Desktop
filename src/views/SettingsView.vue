...
<template>
  ...
  <div v-if="draft" class="settings-stack">
    ...
    <section class="settings-card theme-settings"><div><h2>应用主题</h2><p>内置主题仅包含经过校验的声明式 Token 和布局，不执行脚本。</p></div><div class="theme-editor"><div class="form-grid"><label>外观主题<select v-model="draft.themePack"><option v-for="theme in BUILTIN_THEMES" :key="theme.id" :value="theme.id">{{ theme.name }}</option></select></label><label>主题模式<select v-model="draft.themeMode"><option value="dark">深色</option><option value="light">浅色</option><option value="system">跟随系统</option><option value="custom">自定义配色</option></select></label><label>背景效果<select v-model="draft.themeEffect" :disabled="draft.themeMode === 'system'"><option value="solid">纯色</option><option value="gradient">渐变</option><option value="rainbow">彩虹</option></select></label></div><p v-if="draft.themeMode === 'system'" class="theme-hint">跟随系统使用默认配色；V2 可叠加仅保存在本机的背景图片。</p><div v-if="draft.themeMode === 'custom'" class="color-grid"><label>强调色<input v-model="draft.themeAccent" type="color" /></label><label>辅助色<input v-model="draft.themeSecondary" type="color" /></label><label>背景色<input v-model="draft.themeBackground" type="color" /></label><label>卡片色<input v-model="draft.themeSurface" type="color" /></label></div><div class="form-grid"><button class="secondary" @click="chooseThemeBackground">选择背景图片</button><button v-if="draft.themeBackgroundImage" class="secondary" @click="clearThemeBackground">移除背景</button><label>图片适配<select v-model="draft.themeBackgroundFit"><option value="fill">Fill</option><option value="fit">Fit</option><option value="center">Center</option><option value="stretch">Stretch</option></select></label><label>遮罩 {{ Math.round(draft.themeBackgroundOverlay * 100) }}%<input v-model.number="draft.themeBackgroundOverlay" type="range" min="0" max="0.85" step="0.05" /></label></div><div class="theme-preview" :style="themePreviewStyle"><span></span><strong>{{ draft.themeMode === 'system' ? '跟随系统外观' : '主题预览' }}</strong><small>{{ BUILTIN_THEMES.find((theme) => theme.id === draft.themePack)?.name }} · {{ draft.themeEffect === 'solid' ? '纯色' : draft.themeEffect === 'gradient' ? '渐变背景' : '彩虹背景' }}</small></div></div></section>
    ...
    <section class="settings-card provider-settings"><div><h2>在线图源</h2><p>搜索和刷新会共同查询全部已启用图源，单个失败不影响其他来源。</p></div><div class="provider-list"><button v-for="provider in providers.filter((item) => item.provider !== 'local')" :key="provider.provider" class="provider-row" @click="toggleProvider(provider)"><span><strong>{{ provider.provider === 'wikimedia_commons' ? 'Wikimedia Commons' : 'Wallhaven' }}</strong><small>{{ provider.status }}<template v-if="provider.responseTimeMs"> · {{ provider.responseTimeMs }}ms</template></small></span><em>{{ provider.enabled ? '已启用' : '已停用' }}</em></button></div></section>
