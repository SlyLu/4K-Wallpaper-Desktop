...
/** Converts persisted strategy identifiers into user-facing monitor status text. */
function strategyLabel(value?: string): string {
  return {
    round_robin: "顺序轮询",
    shuffle: "洗牌随机",
    least_recent: "最近未使用优先",
...
<template>
  ...
  <div class="display-grid">
    <button v-for="monitor in monitorStore.monitors" :key="monitor.systemMonitorId" class="display-card" :class="{ selected: selectedMonitorId === monitor.systemMonitorId }" @click="selectedMonitorId = monitor.systemMonitorId">
      ...
      <em v-if="scheduleForMonitor(monitor.systemMonitorId)">已配置 · {{ strategyLabel(schedulerStore.explanations[monitor.systemMonitorId]?.strategy ?? scheduleForMonitor(monitor.systemMonitorId)?.selectionMode) }}</em>
    </button>
  </div>
  <section class="rotation-panel">
    <div class="rotation-heading"><div><p class="eyebrow">ROTATION POOL</p><h2>{{ mode === 'unified' ? '所有显示器统一配置' : '选中显示器独立配置' }}</h2><p v-if="wallpaperStore.selectedCount">使用当前勾选的 {{ wallpaperStore.selectedCount }} 张在线/本地资源。</p><p v-else>未勾选资源，将使用最近设置过的 5 张有效壁纸。</p></div><span v-if="selectedSchedule" class="rotation-state" :class="{ paused: selectedSchedule.paused }">{{ selectedSchedule.paused ? '已暂停' : '运行中' }}</span></div>
    <div class="rotation-section"><div class="rotation-section-title"><strong>基础策略</strong><span>设置切换节奏、顺序和屏幕适配。</span></div><div class="rotation-basic-grid"><label>切换周期<select v-model.number="intervalSeconds"><option :value="600">10 分钟</option><option :value="1800">30 分钟</option><option :value="3600">1 小时</option><option :value="86400">每天</option><option :value="604800">每周</option></select></label><label>选择方式<select v-model="strategy"><option value="round_robin">顺序轮询</option><option value="shuffle">洗牌（每轮不重复）</option><option value="least_recent">最近未使用优先</option><option value="weighted_random">加权随机</option></select></label><label>图片适配<select v-model="fitMode"><option value="fill">填充屏幕</option><option value="fit">完整显示</option><option value="center">原始尺寸居中</option><option value="stretch">拉伸填满</option></select></label></div></div>
    <div v-if="collections.length" class="rotation-section"><div class="rotation-section-title"><strong>集合来源</strong><span>选择集合后优先使用集合；不选则使用勾选壁纸或最近 5 张。</span></div><label class="collection-source-field">参与轮换的集合<select v-model="selectedCollectionIds" multiple><option v-for="collection in collections" :key="collection.id" :value="collection.id">{{ collection.name }}（{{ collection.wallpaperCount }} 张）</option></select><small>按 Ctrl 可选择多个集合。</small></label></div>
    <div class="rotation-section"><div class="rotation-section-title"><strong>生效时间与暂停条件</strong><span>留空开始、结束时间表示全天生效。</span></div><div class="rotation-rule-grid"><label>日期范围<select v-model="rules.dayGroup"><option value="all">每天</option><option value="weekdays">仅工作日</option><option value="weekends">仅周末</option></select></label><label>开始时间<input v-model="rules.startTime" type="time" /></label><label>结束时间<input v-model="rules.endTime" type="time" /></label><label class="rule-toggle"><input v-model="rules.pauseOnBattery" type="checkbox" /><span><strong>电池供电时暂停</strong><small>减少笔记本后台耗电</small></span></label><label class="rule-toggle"><input v-model="rules.pauseOnFullscreen" type="checkbox" /><span><strong>全屏应用时暂停</strong><small>游戏或演示期间不切换</small></span></label></div></div>
    <div class="rotation-footer"><button :disabled="schedulerStore.pending" @click="configure">保存并启用</button><div v-if="selectedSchedule" class="rotation-controls"><button class="secondary" @click="control(selectedSchedule.paused ? 'resume' : 'pause')">{{ selectedSchedule.paused ? '恢复轮换' : '暂停轮换' }}</button><button class="secondary" @click="control('previous')">上一张</button><button class="secondary" @click="control('next')">下一张</button><button class="secondary" @click="control('skip')">跳过当前</button></div></div>
    <div v-if="message || (selectedSchedule && schedulerStore.explanations[selectedSchedule.systemMonitorId]?.lastReason) || selectedSchedule?.lastError" class="rotation-messages"><p v-if="message" class="message">{{ message }}</p><p v-if="selectedSchedule && schedulerStore.explanations[selectedSchedule.systemMonitorId]?.lastReason" class="message">选择原因：{{ schedulerStore.explanations[selectedSchedule.systemMonitorId]?.lastReason }} · 候选 {{ schedulerStore.explanations[selectedSchedule.systemMonitorId]?.candidateCount }} 张 · 队列剩余 {{ schedulerStore.explanations[selectedSchedule.systemMonitorId]?.queuedCount }} 张</p><p v-if="selectedSchedule?.lastError" class="message error">上次错误：{{ selectedSchedule.lastError }}</p></div>
  </section>
</template>
